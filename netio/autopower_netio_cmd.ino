    ...
    if (type != WebServer::HEAD) {
      if (strlen(url_tail)) {
        while (strlen(url_tail)) {
          ...
          if (rc != URLPARAM_EOS) {
            a = atoi(value);
            if (strcmp(name, "cmd") == 0 || strcmp(name, "_cm") == 0) {
              switchOnOff(a, !dev.status[a]); 
              setAutoOff(a);
            }         
            if (strcmp(name, "vac") == 0 || strcmp(name, "_vc") == 0) setVacationMode(a);
            if (strcmp(name, "_rd") == 0) {
              // place netio code here
              server.write(dev.status[a]);
              
            }
            if (strcmp(name, "clk") == 0 || strcmp(name, "_ck") == 0) setTime(getTimeAndDate());
            if (strcmp(name, "add") == 0 || strcmp(name, "_ad") == 0) {
              dev.count[a]++;
              if (dev.count[a] > MAX_EXTEND) dev.count[a] = 0;
            }
          }
        }
        if (name[0] != '_') doRefresh();
      } 
